...
try:
    model, scaler, FEATURE_COLUMNS = load_xgb_pack()
except FileNotFoundError:
    st.error("XGBoost 모델 파일이 없습니다. 먼저 학습을 실행하세요.")
    st.stop()
...
# ==================================================
st.set_page_config(
    page_title="Life Expectancy Predictor (XGBoost)",
    layout="centered"
)

st.title("🌍 기대 수명 예측 서비스 (XGBoost)")
st.caption("XGBoost 기반 기대 수명 예측 모델")
st.markdown("---")
...
if st.button("🔍 기대 수명 예측하기", use_container_width=True):
    ...
    st.markdown(
        ...
        <div style="text-align:center; background-color:#f0f2f6;
                    padding:20px; border-radius:10px; border:2px solid #6a1b9a;">
            <h2>예측된 기대 수명 (XGBoost)</h2>
            <h1 style="color:#6a1b9a; font-size:3.5rem;">
                {prediction:.2f} 년
            </h1>
        </div>
        """,
